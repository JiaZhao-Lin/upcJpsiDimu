...
void anaEvt(Bool_t incHadron = kFALSE, TString hfVetoType="Default")
{
	...
	for (Long64_t jentry = 1; jentry < csTree.GetEntries(); jentry++) 
	{
		...
		hRawCen      ->Fill(cen);
		hVzvsVyvsVxv ->Fill(vtxX, vtxY, vtxZ);
		...
		Int_t  NneutronsInZDC[nDirs]; // 0 - Plus; 1 - Minus;
		memset( NneutronsInZDC, -1, sizeof(NneutronsInZDC) );

		Double_t zdcE = zdcPlus;
		for(Int_t idir = 0; idir < nDirs; idir++) //there are only two directions: ZDC+ and ZDC-
		{
			if(idir == 1) zdcE = zdcMinus;

			NneutronsInZDC[idir] = grabNeutronNum(idir, zdcE); //get number of neutrons from the deposited energies in the ZDCs of the given direction
			
			hZDCvsNeuNum[idir]->Fill(NneutronsInZDC[idir], zdcE);
		}

		hNeuNumMinusvsNeuNumPlus->Fill(NneutronsInZDC[0], NneutronsInZDC[1]);
		...
		for (UInt_t icand = 0; icand < csTree.candSize(); icand++) 
		{
			...
			if(NneutronsInZDC[0] >= 0 && NneutronsInZDC[1] >= 0)
			{
				hMvsPtvsRap_NeuDir[NneutronsInZDC[0]][NneutronsInZDC[1]]     ->Fill(y, pt,     mass);
				hMvsAsyPhivsRap_NeuDir[NneutronsInZDC[0]][NneutronsInZDC[1]] ->Fill(y, asyPhi, mass);
			}
			else
			{
				cout<<"neutron index is wrong ---> "<<"plusIdx: "<<NneutronsInZDC[0]<<"  minusIdx:"<<NneutronsInZDC[1]<<endl;
			}
		}
...
Int_t grabNeutronNum(Int_t dirIdx, Double_t zdc)//get the number of neutrons based the deposited energies in the ZDCs, check it in which cut window
{
