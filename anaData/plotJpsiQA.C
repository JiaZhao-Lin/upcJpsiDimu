	...
	for(Int_t ip=0; ip<nNeus; ip++)
	{
		for(Int_t im=0; im<nNeus; im++)
		{
			for(Int_t irap=0; irap<nDiffRapBins; irap++)
			{
				...
				hLowMassBandPt_NeuDir[ip][im][irap] 
					= (TH1D *)hMvsPtvsRap_NeuDir[ip][im]->ProjectionY(Form("hLowMassBandPt_NeuDir%dp%dm_RapBin%d", ip, im, irap), rapBinLow, rapBinHi, massBinLow, massBinHi);
				...
				hJpsiPt_NeuDir[ip][im][irap] 
					= (TH1D *)hMvsPtvsRap_NeuDir[ip][im]->ProjectionY(Form("hJpsiPt_NeuDir%dp%dm_RapBin%d", ip, im, irap), rapBinLow, rapBinHi, massBinLow, massBinHi);
