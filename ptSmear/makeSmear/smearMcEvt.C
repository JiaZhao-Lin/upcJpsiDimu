...
void bookHistos()
{
	...
	for(Int_t i=0; i<nSmearScan; i++)
	{
		...
		hMvsPtvsRap_Scan[i] = new TH3D(
				Form("hMvsPtvsRap_Scan%d", i), 
				Form("par0 = %5.5f; Rapidity; p_{T} (GeV); m_{#mu#mu} (GeV)", par0), 
				mHistRapBins,  mHistRapLow,  mHistRapHi, 
				mHistPtBins,   mHistPtLow,   mHistPtHi, 
				mHistMassBins, mHistMassLow, mHistMassHi
				);
	}
}
