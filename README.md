...
## 2. neutron decouple
	...
	2. execute 'root -l -b -q plotZDC.C+' to decouple neutron numbers. In UPC Jpsi analysis, 0n0n, 0nXn, XnXn with X>=1 will be used. In this case both the efficiency and purity of 0n and Xn are ~100% according to the fit results. 
...
## 6. Cross-section disentanglement
	...
	2. root -b -q Main.C to produce the result plots. it will produce the disentagled jpsi crosssection and the suppression ratio R.
		-- NOTE --
		a. The Main.C read the already produced root file in /outFiles. To redo the analysis, uncomment the corresponding lines in Main.C
		b. The analysis is activated through a chain of responsibility structured in DoAnalysis(). Since the mid and fwd rap disentanglement are calculated differently from our main method, there are total of three analysis chains. 
		c. Plotting is handled by the class ResultPlotProcessor that selects frame, data point and theory curve. 
		d. Systematics are calculated separately in UncerAnalyzer() which saves a independent root file in outFiles/Result_CMS_SysUncer.root. Systematics plots are handle by UncerPlotProcessor(). It can also be run independently to produce uncertainty plots.
