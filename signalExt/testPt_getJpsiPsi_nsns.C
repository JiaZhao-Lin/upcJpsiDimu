...
void fitFullMassAndPt_4Decouple( const double massLow4Fit=2.6, const double massHig4Fit=4.2, const double ptLow4Fit = 0.0, const double ptHig4Fit = 3.5 )
{
	...
	TFile* infZaochen = new TFile("./templateTest/out4effAndTemp_zy2_wSmr/MassPtTemp_AllSpecs_massWindow_2.95_3.25_.root", "read");
	//TFile* infZaochen = new TFile("./templateTest/out4effAndTemp_zy2_woSmr/MassPtTemp_AllSpecs_massWindow_2.95_3.25_.root", "read");
	//TFile* infZaochen = new TFile("./templateTest/out4effAndTemp_zy1_woSmr/MassPtTemp_AllSpecs_massWindow_2.95_3.25_.root", "read");
	...
	for(int i_ncase=0; i_ncase<NnCases; i_ncase++)
	{
		...
		for(int iy=0; iy<nDiffRapBins+1; iy++)
		{
			...
			if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins) 
			{
				c1 ->SaveAs( Form("outplots/massSpec_4ptFitConstrain_"+nCasesName[i_ncase]+"_iy%d_Symm.pdf",  iy) );
			}
			...
			//------------------------------------------------------------------------------------------------------------
			TH1D* hCohJpsiPtHist;
			TH1D* hInCohJpsiPtHist;
			TH1D* hFeeddownJpsiPtHist;
			TH1D* hQEDPtHist;
			...
			//----------------------------------
			//overwrite the pt template
			//	
			//for test, replace the templates generated by Wei
			//KEY: TH1D	hPt_1619_Pos;1
			//KEY: TH1D	hPt_1921_Pos;1
			//KEY: TH1D	hPt_2124_Pos;1
			//iy: 3 1.6 <y< 1.9
			//iy: 4 1.9 <y< 2.1
			//iy: 5 2.1 <y< 2.4
			//TFile* infwei = new TFile("~/Downloads/pttemplate_new.root", "read");
			//TFile* infwei = new TFile("~/Downloads/pttemplate_new_R77_Pt05.root", "read");
			//TFile* infwei = new TFile("~/Downloads/pttemplate_new_R77_Pt05_Tue.root", "read");
	
			//		TFile* infwei = new TFile("~/Downloads/pttemplate_new_R77_Pt05_comb.root", "read");
			//		if(iy==3) hCohJpsiPtHist = (TH1D*)infwei->Get("hPt_1619_Combo");
			//		if(iy==4) hCohJpsiPtHist = (TH1D*)infwei->Get("hPt_1921_Combo");
			//		if(iy==5) hCohJpsiPtHist = (TH1D*)infwei->Get("hPt_2124_Combo");
			//		//----------------------------------
			...
			// KEY: TH1D	hCohJpsiPt_RapBin2;1	-1.9 < y < -1.6
			...
			//setnames
			hInCohJpsiPtHist->SetName(Form("hInCohJpsiPtHist_ic%d_iy%d",i_ncase, iy));
			hCohJpsiPtHist->SetName(Form("hCohJpsiPtHist_ic%d_iy%d",i_ncase, iy));
			hQEDPtHist->SetName(Form("hQEDPtHist_ic%d_iy%d",i_ncase, iy));
			hFeeddownJpsiPtHist->SetName(Form("hFeeddownJpsiPtHist_ic%d_iy%d",i_ncase, iy));

			//cout<<"before rebin pt"<<endl;
			//cout<<"hPt->GetBinWidth(1): "<<hPt->GetBinWidth(1)<<endl;
			//cout<<"hInCohJpsiPtHist->GetBinWidth(1): "<<hInCohJpsiPtHist->GetBinWidth(1)<<endl;
			//cout<<"hFeeddownJpsiPtHist->GetBinWidth(1): "<<hFeeddownJpsiPtHist->GetBinWidth(1)<<endl;
			//cout<<"hQEDPtHist->GetBinWidth(1): "<<hQEDPtHist->GetBinWidth(1)<<endl;
			//cout<<"hCohJpsiPtHist->GetBinWidth(1): "<<hCohJpsiPtHist->GetBinWidth(1)<<endl;
			...
			cout<<"after rebinning pt"<<endl;
			cout<<"after rebin pt"<<endl;
			cout<<"hPt->GetBinWidth(1): "<<hPt->GetBinWidth(1)<<endl;
			cout<<"hInCohJpsiPtHist->GetBinWidth(1): "<<hInCohJpsiPtHist->GetBinWidth(1)<<endl;
			cout<<"hFeeddownJpsiPtHist->GetBinWidth(1): "<<hFeeddownJpsiPtHist->GetBinWidth(1)<<endl;
			cout<<"hQEDPtHist->GetBinWidth(1): "<<hQEDPtHist->GetBinWidth(1)<<endl;
			...
			const double temPtHigh = 3.00;

			RooPlot *framePt = mPt.frame(Range(ptLow4Fit, temPtHigh), Title(""), Bins(nFramePtBins));//tem test
...
//			drawLatex(0.35, 0.45+textDy2, Form("#frac{#sigma^{Coh}_{J/#psi}}{dy} = %.3f #pm %.3f (mb)", xsecValue[i_ncase][iy], xsecError[i_ncase][iy] ),                                     mTextFont, 0.035, mTextColor);
			...
			RooPlot *framePtPull = mPt.frame( Range(ptLow4Fit, temPtHigh), Title(""), Bins(nFramePtBins) );
