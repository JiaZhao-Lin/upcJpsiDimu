...
Int_t    mLineWidth = 1;
Int_t    cohPsiColor       = kBlue,      cohPsiStyle      = 1;
Int_t    incohPsiColor     = kViolet-1,  incohPsiStyle    = 2;
Int_t    dissoPsiColor     = kRed,       dissoPsiStyle    = 2;
//Int_t    feeddownJpsiColor  = kAzure+10,  feeddownJpsiStyle = 1;
Int_t    qedColor = kGreen-3, qedStyle = 1;

const Double_t mJpsi_PDG = 3.0969, mPsi_PDG = 3.686097;
const double ptLow4MFit = 0.0;
const double ptHig4MFit = 2.5;

void fitPsi(Bool_t incHadron = kFALSE, TString hfVetoType="Default", Double_t massLow=2.6, Double_t massHi=4.2, Double_t ptLow=0.00, Double_t ptHi=2.5)//Double_t ptHi=3.6)
{
	...
	{
		//Int_t ptBinLow         = hMvsPtvsRap->GetYaxis()->FindBin( 0.              + mTinyNum );
		//Int_t ptBinHi          = hMvsPtvsRap->GetYaxis()->FindBin( mPtCut4Coh      - mTinyNum );
		Int_t ptBinLow         = hMvsPtvsRap->GetYaxis()->FindBin( ptLow             + mTinyNum );
		Int_t ptBinHi          = hMvsPtvsRap->GetYaxis()->FindBin( ptHi              - mTinyNum );
		...
		Int_t PsiMBinLow       = hMvsPtvsRap->GetZaxis()->FindBin( mPsiMassLow     + mTinyNum );
		Int_t PsiMBinHi        = hMvsPtvsRap->GetZaxis()->FindBin( mPsiMassHi      - mTinyNum );

		Int_t leftSideMBinLow  = hMvsPtvsRap->GetZaxis()->FindBin( mPsiMassLow-0.15 + mTinyNum );
		Int_t leftSideMBinHi   = hMvsPtvsRap->GetZaxis()->FindBin( mPsiMassLow-0.05 - mTinyNum );
		Int_t rightSideMBinLow = hMvsPtvsRap->GetZaxis()->FindBin( mPsiMassHi+0.05  + mTinyNum );
		Int_t rightSideMBinHi  = hMvsPtvsRap->GetZaxis()->FindBin( mPsiMassHi+0.25  - mTinyNum );
		
		hPt_Rap[irap]    = (TH1D *)hMvsPtvsRap->ProjectionY(Form("hPt_RapBin%d",   irap), rapBinLow, rapBinHi, PsiMBinLow,  PsiMBinHi );
	...
	const int mRebPt   = 2, mRebPt_Rap   = 5;
	...
	//use Jpsi mass shape to constrain Psi shape
	TFile *inf_Temps4massFit = TFile::Open("../simulation/effAndTemp/MassPtTemp_AllSpecs_massWindow_2.95_3.25_.root");

	TF1  *fCohJpsiTemp  = (TF1  *) inf_Temps4massFit->Get("fCohJpsiTemp");
	TH1D *hJpsiMassHist = (TH1D *) inf_Temps4massFit->Get("hCohJpsiMass");
	TH1D *hQEDMassHist  = (TH1D *) inf_Temps4massFit->Get("hLowMassGammaGammaMass");
	...
	RooRealVar nQED( "nQED","nQED", 3.e4,                0, 8.e5);
	...
	const double RationPsi2nJpsi  = nPsiValue/nJpsiValue;

	cout<<"mtrx_cov[8][9]: "<<mtrx_cov[7][8]<<endl;
	const double RationPsi2nJpsiErr  = RationPsi2nJpsi*sqrt( pow(nJpsiError/nJpsiValue, 2) + pow(nPsiError/nPsiValue, 2) - (2.*mtrx_cov[7][8])/(nPsiValue*nJpsiValue)  );

	cout<<"RationPsi2nJpsi: "<<RationPsi2nJpsi<<" +/- "<<RationPsi2nJpsiErr<<endl;
	...
	Double_t chi2ndf = frameMass->chiSquare("totMassPdf_Norm[mMass]", "h_dataMass", 10);   //Jpsi+Psi fit

	frameMass->Draw() ;
	
	drawLatex(0.60, 0.86, Form("%1.1f < |y^{#mu#mu}| < %1.1f",    mRapLow[nRapBins/2], mRapHi[nRapBins-1]), mTextFont, 0.05, mTextColor);
	drawLatex(0.60, 0.80, Form("%1.0f < p_{T}^{#mu#mu} < %1.1f GeV/c",  ptLow,           ptHi), mTextFont, 0.05, mTextColor);
	drawLatex(0.60, 0.72, Form("#chi^{2}/ndf = %1.1f", chi2ndf),                                 mTextFont, mTextSize, mTextColor);
	drawLatex(0.60, 0.66, Form("N_{J/#psi} = %d #pm %d",   TMath::Nint(nJpsi.getVal()), TMath::Nint(nJpsi.getError())), mTextFont, mTextSize, mTextColor);
	drawLatex(0.60, 0.60, Form("N_{#psi(2S)} = %d #pm %d", TMath::Nint(nPsi.getVal()),  TMath::Nint(nPsi.getError()) ), mTextFont, mTextSize, mTextColor);
	drawLatex(0.60, 0.55, Form("N_{QED} = %d #pm %d",      TMath::Nint(nQED.getVal()),  TMath::Nint(nQED.getError()) ), mTextFont, mTextSize, mTextColor);
	drawLatex(0.60, 0.45, Form("#frac{N_{#psi(2S)}}{N_{J/#psi}} = %.3f #pm %.4f", nPsi.getVal()/nJpsi.getVal(), RationPsi2nJpsiErr ), mTextFont, mTextSize, mTextColor);
	
	//----------------------------------------------------------------------------------------------------------------------------------------------------
	c1->SaveAs(Form("%s/massSpec_4Psi.pdf", dirName.Data()));
	//----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	TFile *inf_Temps4ptFit = TFile::Open("../simulation/effAndTemp/MassPtTemp_AllSpecs_massWindow_3.50_3.80_.root");
	TH1D *hCohPsiPtHist    = (TH1D *)inf_Temps4ptFit->Get("hCohPsi2SPt"           );
	TH1D *hInCohPsiPtHist  = (TH1D *)inf_Temps4ptFit->Get("hInCohPsi2SPt"         );
	TH1D *hQEDPtHist       = (TH1D *)inf_Temps4ptFit->Get("hLowMassGammaGammaPt");
	...
	//--------------------------------------------------------------------------------------
	//--------------------------------------------------------------------------------------
	hCohPsiPtHist   ->RebinX( mRebPt );
	RooDataHist hCohPsiPtRooHist(     "hCohPsiPtRooHist",      "hCohPsiPtRooHist",      mPt, hCohPsiPtHist);
	RooHistPdf  cohPsiPdf(            "cohPsiPdf",             "cohPsiPdf",             mPt, hCohPsiPtRooHist,      0);
	//--------------------------------------------------------------------------------------
	//--------------------------------------------------------------------------------------
	hInCohPsiPtHist ->RebinX( mRebPt );
	RooDataHist hInCohPsiPtRooHist(   "hInCohPsiPtRooHist",    "hInCohPsiPtRooHist",    mPt, hInCohPsiPtHist);
	RooHistPdf  incohPsiPdf(          "incohPsiPdf",           "incohPsiPdf",           mPt, hInCohPsiPtRooHist,    0);
...
//	RooHistPdf  qedPtPdf("qedPtPdf", "qedPtPdf", mPt, hQEDPtRooHist, 0);
	...
	RooRealVar bpd("bpd", "bpd", 1.79, 0, 5);
	RooRealVar npd("npd", "npd", 3.58, 0, 5);
	RooGenericPdf *dissoPsiPdf = new RooGenericPdf("dissoPsiPdf", "dissoPsiPdf", "mPt*TMath::Power(1+(bpd/npd)*mPt*mPt, -npd)", RooArgSet(mPt, bpd, npd));

	fQED->SetParameters(mP0.getVal(), mP1.getVal(), mP2.getVal(), mP3.getVal());
	Double_t mQEDFrac = fQED->Integral(mPsiMassLow, mPsiMassHi) / fQED->Integral(massLow, massHi);

	RooRealVar  nCohPsi(     "nCohPsi",       "nCohPsi",      2.0e3, 0, 1.e5);
	RooRealVar  nInCohPsi(   "nInCohPsi",     "nInCohPsi",    1.2e1, 0, 5.e2);
	RooRealVar  nDissoPsi(   "nDissoPsi",     "nDissoPsi",    1.e4,  0, 5.e3);
	...
	RooAddPdf totPtPdf("totPtPdf", "totPtPdf", 
			RooArgList(cohPsiPdf, incohPsiPdf, *dissoPsiPdf, qedPtPdf ),
			//RooArgList(*cohPsiPdf, incohPsiPdf, *dissoPsiPdf, qedPtPdf ),
			RooArgList(nCohPsi,    nInCohPsi,    nDissoPsi,   nQEDBg)  );
	...
	//calculate the fI = inCoh/Coh within pt<0.20;
	const double frac_InCohPsiInPtCut = hInCohPsiPtHist->Integral(0,hInCohPsiPtHist->FindBin(mPtCut4Coh))/hInCohPsiPtHist->Integral();
	const double frac_CohPsiInPtCut   = hCohPsiPtHist  ->Integral(0,hCohPsiPtHist->FindBin(mPtCut4Coh)  )/hCohPsiPtHist->Integral();
	const double nInCohPsiInPtCut     = nInCohPsi.getVal() * frac_InCohPsiInPtCut;
	const double nCohPsiInPtCut       = nCohPsi.getVal()   * frac_CohPsiInPtCut;
	const double fI = nInCohPsiInPtCut/nCohPsiInPtCut;
	...
	//	totPtPdf.plotOn(framePt, Components(RooArgSet(*cohPsiPdf)),      LineColor(cohPsiColor),      LineStyle(cohPsiStyle),      LineWidth(mLineWidth));
	totPtPdf.plotOn(framePt, Components(RooArgSet(cohPsiPdf)),      LineColor(cohPsiColor),      LineStyle(cohPsiStyle),      LineWidth(mLineWidth));
	totPtPdf.plotOn(framePt, Components(RooArgSet(incohPsiPdf)),    LineColor(incohPsiColor),    LineStyle(incohPsiStyle),    LineWidth(mLineWidth));
	totPtPdf.plotOn(framePt, Components(RooArgSet(*dissoPsiPdf)),   LineColor(dissoPsiColor),    LineStyle(dissoPsiStyle),    LineWidth(mLineWidth));
	...
	gPad->SetLogy(1);
	framePt->GetYaxis()->SetRangeUser(0.30, hPt->GetMaximum()*5);
	framePt->Draw() ;
	...
	drawLatex(0.18, 0.75, Form("%.1f < M_{#mu#mu} < %.1f GeV/c^{2}", mPsiMassLow, mPsiMassHi),   mTextFont, mTextSize, mTextColor );

	TLegend  *leg =  new TLegend(0.50, 0.45, 0.84, 0.80);
	leg->SetFillStyle(0);
	leg->SetFillColor(0);
	...
	const TString curveName[4]  = {"totPtPdf_Norm[mPt]_Comp[cohPsiPdf]","totPtPdf_Norm[mPt]_Comp[incohPsiPdf]", "totPtPdf_Norm[mPt]_Comp[dissoPsiPdf]", "totPtPdf_Norm[mPt]_Comp[qedPtPdf]"};
	const TString curveTitle[4] = {"Coherent #psi(2S)", "Incoherent #psi(2S)", "Incoherent #psi(2S) with n-disso.", "#gamma#gamma #rightarrow #mu#mu"};
	...
	//
	c2->SaveAs(Form("%s/ptSpec_4Psi.pdf", dirName.Data()));
