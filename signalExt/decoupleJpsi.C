...
Int_t    incohJpsiColor     = kViolet-1,  incohJpsiStyle    = 2;
Int_t    dissoJpsiColor     = kRed,       dissoJpsiStyle    = 2;
...
void decoupleJpsi(Bool_t incHadron = kFALSE, TString hfVetoType="Default", Double_t massLow=2.5, Double_t massHi=4.5, Double_t ptLow=0.00, Double_t ptHi=3.0)//Double_t ptHi=3.6)
{
	...
	{
		Int_t ptBinLow         = hMvsPtvsRap->GetYaxis()->FindBin( 0.00            + mTinyNum );
		Int_t ptBinHi          = hMvsPtvsRap->GetYaxis()->FindBin( 0.20            - mTinyNum ); //only look at pt<0.2 GeV/c for Coh signals, for fD
	...
	//RooRealVar RatioPsi2Jpsi("RatioPsi2Jpsi", "RatioPsi2Jpsi", 0.05, 0, 1.0);
	...
	frameMass ->SetAxisRange(1.,7.e2, "y");
	...
	//Double_t chi2ndf = frameMass->chiSquare("totMassPdf_Norm[mMass]", "h_dataMass", 9); //Jpsi only fit
	...
	drawLatex(0.60, 0.84, Form("%1.1f < |y| < %1.1f",  mRapLow[nRapBins/2], mRapHi[nRapBins-1]), mTextFont, 0.05, mTextColor);
	...
	RooConstVar mu3(    "mu3",    "mu3",     0.000007    );
	RooConstVar sigma3( "sigma3", "sigma3",  0.03261 );
	RooConstVar ratioSigma("ratioSigma", "ratioSigma", 1.59 );
	RooConstVar a3( "a3", "a3",  0.49 );
	RooGenericPdf *cohJpsiPdf = new RooGenericPdf("cohJpsiPdf", "cohJpsiPdf", "mPt*(a3*TMath::Exp(-pow(mPt-mu3,2)/(2.*pow(sigma3,2))) + (1.-a3)*TMath::Exp( -pow(mPt-mu3,2)/(2.*pow(sigma3*ratioSigma,2)) ))", RooArgSet(mPt, mu3,sigma3,ratioSigma, a3) );
	...
	RooRealVar  nInCohJpsi(   "nInCohJpsi",     "nInCohJpsi",    1.2e2, 0, 5.e4);
