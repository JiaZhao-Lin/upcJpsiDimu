...
TMatrixD getPileUp_CorrFactor()
{
	...
	const double MigrationMatrixElement[nDmatrixs*nDmatrixs] = {	P00_00,	0,		0,		0,
											   						P0X_00,	P0X_0X,	0,		0,
											   						PX0_00,	0,		PX0_X0,	0,
...
void PileUp_Corr( double NJpsi_inMFit[][7], double NerrJpsi_inMFit[][7], const double nDiffRapBins )
{
	...
	for (int iy = 0; iy < nDiffRapBins; ++iy)
	{
		...
		//---------------------------------------Yang Shuai Correction Method------------------------------------
		double tem[nDmatrixs] = {NJpsi_inMFit[1][iy],	NJpsi_inMFit[2][iy],	NJpsi_inMFit[3][iy],	NJpsi_inMFit[5][iy]};
		TMatrixD N_Meastured_iy(nDmatrixs,	1, tem); 
		TMatrixD N_True_iy = MigrationMatrixInvert * N_Meastured_iy;
		...
		NJpsi_inMFit[4][iy] = N_True_iy[1][0] + N_True_iy[2][0];
		...
		NerrJpsi_inMFit[4][iy] = sqrt( pow(NerrJpsi_inMFit[2][iy],2) + pow(NerrJpsi_inMFit[3][iy],2) );
		//-------------------------------------------------------------------------------------------------------
	}
}
