...
void plotShadowingRatio()
{
	...
	TGraphErrors* ge_ALICE 	= new TGraphErrors(ALICE_x.size(), &ALICE_x[0], &ALICE_R[0], 0, &ALICE_R_Err[0]);
	...
	ge_ALICE->SetMarkerStyle(24);
	ge_ALICE->SetMarkerColor(4);
	ge_ALICE->SetLineColor(4);
	ge_ALICE->Draw("pesame");
	...
	TLegend  *leg =  new TLegend(0.62, 0.25, 0.88, 0.50);
	leg->SetFillStyle(0);
	leg->SetFillColor(0);
	leg->SetTextSize(0.055);
	leg->AddEntry(ge_CMS,    "CMS",        "lp");
	leg->AddEntry(ge_ALICE,  "ALICE",      "lp");
	leg->Draw("same");
