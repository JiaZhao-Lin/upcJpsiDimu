...
void plotShadowingRatio( )
{
	...
	const int nALICE =2; 
	const double x_ALICE[nALICE]    = {1.e-3, 2.2e-2};
	const double R_ALICE[nALICE]    = {0.65,  0.80};
	const double Rerr_ALICE[nALICE] = {0.03,  0.03};
	
	TGraphErrors* ge_ALICE_Run2 = new TGraphErrors(nALICE, x_ALICE, R_ALICE, 0, Rerr_ALICE);
