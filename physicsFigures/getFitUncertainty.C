...
void getFitUncertainty()
{
	...
	//-----------------------------------W Unicertainty----------------------------------
	auto c1 = new TCanvas();
	auto Sigma_FitUncer	= new TH2D("Sigma_FitUncertainty", ";W_{#gammaPb} (GeV);#sigma Uncer.(%);", 10,14,520, 10,0,7);
	Sigma_FitUncer->Draw();
	...
	auto c2 = new TCanvas();
	c2->SetLogx();
	auto R_FitUncer	= new TH2D("R_FitUncertainty", ";x;R^{Pb}_{g} Uncer.(%);", 10,4.0e-5,1.1e-2, 10, 0, 5);
	R_FitUncer->Draw();
